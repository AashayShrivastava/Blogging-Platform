...
@app.route("/post/<string:query>", methods=['GET', 'POST']) 
def filter_posts(query):
    form = FilterForm()
    page1=request.args.get('page',1,type=int)

    # Query the Post model and apply filtering
    post_query = Post.query.filter(
        (Post.title.ilike(f'%{query}%')) |
        (Post.content.ilike(f'%{query}%')) |
        (Post.author.has(username=query))
    ).order_by(Post.id.desc())

    # Paginate the filtered results
    post1 = post_query.paginate(page=page1, per_page=3)

    # Check if there are filtered posts
    if post1.items:
        return render_template("user_post1.html", posts={'post':post1, 'q': query}, pagination=post1)
    else:
        return "The word you entered is not on the current page"
